...
import { buildPlanReviewSnapshot, PLAN_REVIEW_SECTION_KEYS, stableStringify, type PlanReviewSectionKey, type PlanReviewSource } from '../../lib/planReview';
...
export function PlanReviewBookingCard({ controller, accountState, source, t, consentLocale, onUpgrade }: Props) {
  ...
  const fingerprint = useMemo(() => stableStringify(snapshot), [snapshot]);
  ...
  async function submit() {
    ...
    const result = await controller.requestPlanReview({ startsAt, timezone: detectedTimeZone(), durationMinutes: 60, purpose: 'plan_review',
      focusReason: `${k(`purposes.${purpose}`)}${focus.trim() ? ` — ${focus.trim()}` : ''}`,
      questions: questions.split('\n').map((item) => item.trim()).filter(Boolean).slice(0, 10), selectedSections: selected,
