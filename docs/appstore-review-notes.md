...
Information → Notes. Fill the bracketed placeholders first. Also complete the
demo-account and contact fields in that same section.

> ⚠️ Before submitting, replace every [BRACKET] with a real value and CREATE the
> three demo accounts in the live app so the reviewer can actually sign in.
...
EXTERNAL PAYMENT FOR 1:1 COACHING (Guideline 3.1.3(d))
The "Book 1:1 coaching" feature schedules a real-time, one-to-one video coaching
session between two individuals and collects payment outside IAP via PayPal. This
is a real-time person-to-person service as described in Guideline 3.1.3(d), which
permits payment methods other than IAP. Group sessions and digital subscriptions
use IAP.
...
Some accounts are connected to a provider organization (e.g., an intervention
practice). Those families access services through their provider and see the
provider's branding. The FREEDOM2026 code above demonstrates this flow.
...
Terms of Service: [https://soberhelpline.com/terms]
...
[Matt Brown] · [matt@soberhelpline.com] · [phone]

---

## Pre-submission checklist (do these before hitting Submit)

- [ ] Created the 3 demo accounts in the LIVE app and verified each signs in.
- [ ] Filled every [BRACKET] above with real values.
...
- [ ] Demo PayPal flow works (or note it's external and explain, as above).
...
2. External payment (PayPal) unexplained → the 3.1.3(d) paragraph above pre-empts it.
